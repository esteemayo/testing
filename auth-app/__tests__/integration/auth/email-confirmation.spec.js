...
describe('The register process(Email confirmation process)', () => {
    const EMAIL_CONFIRMATION_ENDPOINT = '/api/v1/auth/emails/confirm';
    ...
    it('returns a 422 if token is invalid', async () => {
        const response = await app().post(EMAIL_CONFIRMATION_ENDPOINT).send({token: 'xxx'});
    ...
    it('confirms a user email', async () => {
        ...
        const response = await app().post(EMAIL_CONFIRMATION_ENDPOINT).send({
